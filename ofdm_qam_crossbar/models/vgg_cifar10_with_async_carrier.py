...
from MODEM import fdm_num, demodulation_matrix
...
class AlexNetOWT_BN(nn.Module):
    ...
    def add_async_error_in_output(self, tensor, ratio_sigma):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.add_async_error_in_output(x, sigma_value)
        x = self.bn1(x)
        x = self.relu1(x)
        x = self.conv2(x)
        x = self.add_async_error_in_output(x, sigma_value)
    ...
    def forward_the_first_layer(self, x):
        x = self.conv1(x)
        print("start add")
        y = self.add_async_error_in_output(x, self.sigma)
        print("end add")
        return x,y

def vgg_cifar10_with_async_carrier(**kwargs):
